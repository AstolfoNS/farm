...
public final class SeedStageRuleSupport {
    ...
    public static ResolvedStageRule resolve(SeedType seedType, List<SeedGrowthStage> stages) {
        ...
        short regrowStageIndex = resolveRegrowStageIndex(seedType, stages, firstStageIndex, harvestStageIndex);
        return new ResolvedStageRule(firstStageIndex, harvestStageIndex, witherStageIndex, regrowStageIndex);
    }

    public static int calculateSecondsUntilStage(List<SeedGrowthStage> stages, Short startStageIndex, Short targetStageIndex, BigDecimal multiplier) {
        if (stages == null || stages.isEmpty() || targetStageIndex == null) {
            return 0;
        }
        int startPos = findStagePos(stages, startStageIndex == null ? stages.getFirst().getStageIndex() : startStageIndex);
        ...
        BigDecimal safeMultiplier = multiplier == null || multiplier.compareTo(BigDecimal.ZERO) <= 0
                ? BigDecimal.ONE
                : multiplier;
        BigDecimal adjusted = BigDecimal.valueOf(totalRawSeconds).divide(safeMultiplier, 0, RoundingMode.HALF_UP);
    ...
    private static short resolveHarvestStageIndex(SeedType seedType, List<SeedGrowthStage> stages, short witherStageIndex) {
    ...
    private static short resolveRegrowStageIndex(SeedType seedType, List<SeedGrowthStage> stages, short firstStageIndex, short harvestStageIndex) {

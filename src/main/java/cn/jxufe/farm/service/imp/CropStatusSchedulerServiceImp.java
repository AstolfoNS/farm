...
public class CropStatusSchedulerServiceImp implements CropStatusSchedulerService {
  ...
  public CropStatusSchedulerServiceImp(
    ...
    this.schedulerExecutor =
        Executors.newFixedThreadPool(safeParallelism, new SchedulerThreadFactory());
  }
  ...
  private CropRuntimeContext buildRuntimeContext(List<UserCrop> crops) {
    ...
    List<Long> plotIds =
        crops.stream().map(UserCrop::getPlotId).distinct().collect(Collectors.toList());
    List<Long> seedTypeIds =
        crops.stream().map(UserCrop::getSeedTypeId).distinct().collect(Collectors.toList());

    List<UserPlot> plots =
        plotIds.isEmpty() ? List.of() : userPlotDao.findByIdInAndIsDeletedFalse(plotIds);
    context.plotById =
        plots.stream().collect(Collectors.toMap(UserPlot::getId, v1 -> v1, (a1, b1) -> a1));

    List<Long> soilTypeIds =
        plots.stream().map(UserPlot::getSoilTypeId).distinct().collect(Collectors.toList());
    List<SoilType> soils =
        soilTypeIds.isEmpty() ? List.of() : soilTypeDao.findByIdInAndIsDeletedFalse(soilTypeIds);
    context.soilById =
        soils.stream().collect(Collectors.toMap(SoilType::getId, v -> v, (a, b) -> a));

    List<SeedType> seedTypes =
        seedTypeIds.isEmpty() ? List.of() : seedTypeDao.findByIdInAndIsDeletedFalse(seedTypeIds);
    context.seedTypeById =
        seedTypes.stream().collect(Collectors.toMap(SeedType::getId, v -> v, (a, b) -> a));

    List<SeedGrowthStage> stageRows =
        seedTypeIds.isEmpty()
            ? List.of()
            : seedGrowthStageDao
  ...
  private PartitionResult processPartition(
      List<UserCrop> partition, CropRuntimeContext context, OffsetDateTime now) {
    List<CropRuntimeChange> changes = new ArrayList<>();
    Set<Long> changedUserIds = new HashSet<>();
    ...
    for (UserCrop crop : partition) {
      ...
      Short resolvedRuntimeStatus =
          stageEvolution.runtimeStatus == null ? runtimeStatus : stageEvolution.runtimeStatus;

      boolean statusChanged =
          resolvedRuntimeStatus != null && !resolvedRuntimeStatus.equals(crop.getGrowStatus());
  ...
  private StageEvolutionResult evolveStageAndBug(
      UserCrop crop, CropRuntimeContext context, Short runtimeStatus, OffsetDateTime now) {
    ...
    SeedStageRuleSupport.ResolvedStageRule stageRule =
        SeedStageRuleSupport.resolve(seedType, stages);
    ...
    while (pos < stages.size() - 1) {
      int adjustedDuration =
          adjustDurationSeconds(stages.get(pos).getDurationSeconds(), multiplier);
      boolean reachedNextStage =
          adjustedDuration <= 0 || !now.isBefore(cursor.plusSeconds(adjustedDuration));
      if (!reachedNextStage) {
        break;
      }
    ...
    result.nextStageStartedAt = nextStageIndex == currentStageIndex ? stageStartAt : cursor;
  ...
  private ApplyResult applyChanges(
      List<UserCrop> allCrops, List<CropRuntimeChange> changes, OffsetDateTime now) {
    ...
    for (CropRuntimeChange change : changes) {
      ...
      if (change.nextExpectedWitheredAt != null
          && !change.nextExpectedWitheredAt.equals(crop.getExpectedWitheredAt())) {
  ...
  private BigDecimal resolveSoilMultiplier(Long plotId, CropRuntimeContext context) {
    ...
    if (soilType == null
        || soilType.getGrowSpeedMultiplier() == null
        || soilType.getGrowSpeedMultiplier().compareTo(BigDecimal.ZERO) <= 0) {
      return BigDecimal.ONE;
    }
  ...
  private String buildBugSpawnSnapshot(CropRuntimeChange change) {
    ...
    return "{\"reason\":\"STAGE_TRANSITION\""
        ...
        + enteredStagesJson
        + ",\"bugAddedCount\":"
        + change.bugAddedCount
        + ",\"bugCountBefore\":"
        + change.bugCountBefore
        + ",\"bugCountAfter\":"
        + safeShort(change.nextBugCount)
        + "}";
  }

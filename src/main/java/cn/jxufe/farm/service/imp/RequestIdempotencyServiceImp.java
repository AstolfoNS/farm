...
public class RequestIdempotencyServiceImp implements RequestIdempotencyService {
  ...
  public RequestIdempotencyServiceImp(
      RequestIdempotencyDao requestIdempotencyDao, ObjectMapper objectMapper) {
  ...
  public <T> T getCachedSuccessResult(
      Long userId, String bizType, String requestId, Class<T> responseType) {
    String normalizedRequestId = normalizeRequestId(requestId);
    Optional<RequestIdempotency> optional =
        requestIdempotencyDao.findByUserIdAndBizTypeAndRequestIdAndIsDeletedFalse(
            userId, bizType, normalizedRequestId);
    if (optional.isEmpty()) {
      return null;
    }

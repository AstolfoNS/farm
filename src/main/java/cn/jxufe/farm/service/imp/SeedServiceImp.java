...
public class SeedServiceImp implements SeedService {
    ...
    public PageResult<SeedGridVO> pageSeedTypes(SeedTypeQueryDTO query) {
        ...
        Pageable pageable = PageRequest.of(pageNo - 1, pageSize, Sort.by(direction, safeSeedSortField(request.getSort())));

        Page<SeedType> seedPage = seedTypeDao.findByIsDeletedFalseAndNameContainingIgnoreCase(gameplayCoreService.safeString(request.getName()).trim(), pageable);

        Map<Long, SeedQuality> qualityMap = buildSeedQualityMap();
        Map<Integer, String> soilNameByBitCode = buildSoilNameByBitCode();

        List<SeedGridVO> records = seedPage.getContent().stream()
    ...
    public PageResult<SeedShopItemVO> pageSeedShop(SeedShopQueryDTO query) {
        ...
        List<SeedShopItemVO> allItems = seedTypeDao.findByIsDeletedFalseOrderByIdAsc().stream()
                .filter(s -> nameKeyword.isEmpty() || gameplayCoreService.safeString(s.getName()).toLowerCase().contains(nameKeyword))
                .filter(s -> request.getSeedQualityId() == null || request.getSeedQualityId().equals(s.getSeedQualityId()))
    ...
    public SeedShopBuyResultVO buySeed(SeedShopBuyDTO params) {
        ...
        Long seedTypeId = ServiceGuardUtils.requirePositive(params.getSeedTypeId(), BizErrorCode.PARAM_INVALID, "种子类型ID无效");
        long buyQuantity = gameplayCoreService.defaultLong(params.getQuantity(), 0L);
        if (buyQuantity <= 0) throw new ServiceException(BizErrorCode.AMOUNT_INVALID, "购买数量必须大于0");

        SeedShopBuyResultVO cached = requestIdempotencyService.getCachedSuccessResult(userId, "BUY_SEED", params.getRequestId(), SeedShopBuyResultVO.class);
        ...
        try {
            ...
            if (totalCost == Long.MAX_VALUE) throw new ServiceException(BizErrorCode.AMOUNT_OVERFLOW, "购买金额计算溢出");
            ...
            UserSeed userSeed = userSeedDao.findByUserIdAndSeedTypeIdAndIsDeletedFalse(userId, seedTypeId)
                    .orElseGet(() -> {
            ...
            if (userSeed.getId() == null) userSeedDao.save(userSeed);

            long frozenSeedQuantity = gameplayCoreService.defaultLong(userSeed.getFrozenQuantity(), 0L);
            ...
            long afterSeedQuantity = gameplayCoreService.defaultLong(validateUserSeed(userId, seedTypeId).getQuantity(), 0L);
            long beforeSeedQuantity = afterSeedQuantity - buyQuantity;

            String bizId = seedTypeId + ":" + now.toEpochSecond();
            userAssetFlowDao.save(gameplayCoreService.buildAssetFlow(userId, "COIN", "EXPENSE", totalCost, beforeCoin, afterCoin, "BUY_SEED", bizId,
                    now, "{\"seedTypeId\":" + seedTypeId + ",\"buyQuantity\":" + buyQuantity + "}"));

            userInventoryFlowDao.save(gameplayCoreService.buildInventoryFlow(userId, "SEED", seedTypeId, "INCOME", buyQuantity, beforeSeedQuantity, afterSeedQuantity,
                    frozenSeedQuantity, frozenSeedQuantity, "BUY_SEED", bizId, now, "{\"seedTypeId\":" + seedTypeId + ",\"totalCostCoin\":" + totalCost + "}"));
    ...
    public SeedShopSellFruitResultVO sellFruit(SeedShopSellFruitDTO params) {
        ...
        if (sellQuantity <= 0) throw new ServiceException(BizErrorCode.AMOUNT_INVALID, "出售数量必须大于0");

        SeedShopSellFruitResultVO cached = requestIdempotencyService.getCachedSuccessResult(userId, "SELL_FRUIT", params.getRequestId(), SeedShopSellFruitResultVO.class);
        if (cached != null) return cached;
        ...
        try {
            validateUser(userId);
            SeedType seedType = validateSeedType(seedTypeId);
            UserFruit userFruit = ServiceGuardUtils.requirePresent(
            ...
            long availableFruit = gameplayCoreService.defaultLong(userFruit.getQuantity(), 0L) - frozenFruitQuantity;
            if (availableFruit < sellQuantity) throw new ServiceException(BizErrorCode.FRUIT_NOT_ENOUGH, "果实库存不足");
            ...
            if (totalIncomeCoin == Long.MAX_VALUE) throw new ServiceException(BizErrorCode.AMOUNT_OVERFLOW, "出售金额计算溢出");

            OffsetDateTime now = OffsetDateTime.now();
            if (userFruitDao.decreaseAvailableQuantityIfEnough(userFruit.getId(), sellQuantity, userId, now) <= 0) {
            ...
            long afterFruitQuantity = gameplayCoreService.defaultLong(validateUserFruit(userId, seedTypeId).getQuantity(), 0L);
            long beforeFruitQuantityAccurate = afterFruitQuantity + sellQuantity;
            long afterCoin = gameplayCoreService.defaultLong(validateUser(userId).getCoin(), 0L);
            long beforeCoin = afterCoin - totalIncomeCoin;

            String bizId = seedTypeId + ":" + now.toEpochSecond();
            userInventoryFlowDao.save(gameplayCoreService.buildInventoryFlow(userId, "FRUIT", seedTypeId, "EXPENSE", sellQuantity, beforeFruitQuantityAccurate, afterFruitQuantity,
                    frozenFruitQuantity, frozenFruitQuantity, "SELL_FRUIT", bizId, now, "{\"seedTypeId\":" + seedTypeId + ",\"unitFruitPrice\":" + unitFruitPrice + "}"));

            userAssetFlowDao.save(gameplayCoreService.buildAssetFlow(userId, "COIN", "INCOME", totalIncomeCoin, beforeCoin, afterCoin, "SELL_FRUIT", bizId,
                    now, "{\"seedTypeId\":" + seedTypeId + ",\"sellQuantity\":" + sellQuantity + "}"));
    ...
    public PageResult<SeedShopTradeRecordVO> pageShopTrades(SeedShopTradeQueryDTO query) {
        ...
        if (request.getTradeType() != null && !request.getTradeType().isBlank() && tradeTypeFilter.isEmpty()) {
        ...
        Map<String, UserInventoryFlow> inventoryByBizId = Stream.concat(
                userInventoryFlowDao.findByUserIdAndItemTypeAndIsDeletedFalseOrderByOccurredAtDesc(userId, "SEED").stream(),
                userInventoryFlowDao.findByUserIdAndItemTypeAndIsDeletedFalseOrderByOccurredAtDesc(userId, "FRUIT").stream()
        ).filter(f -> f.getBizId() != null).collect(Collectors.toMap(UserInventoryFlow::getBizId, Function.identity(), (v1, v2) -> v1));

        Map<Long, SeedType> seedTypeMap = buildSeedTypeMap();

        List<SeedShopTradeRecordVO> records = assetFlows.stream()
                .filter(flow -> "BUY_SEED".equalsIgnoreCase(flow.getBizType()) || "SELL_FRUIT".equalsIgnoreCase(flow.getBizType()))
                .filter(flow -> tradeTypeFilter.isEmpty() || tradeTypeFilter.equals("BUY_SEED".equalsIgnoreCase(flow.getBizType()) ? "BUY" : "SELL"))
                .map(flow -> {
                    String tradeType = "BUY_SEED".equalsIgnoreCase(flow.getBizType()) ? "BUY" : "SELL";
                    UserInventoryFlow invFlow = inventoryByBizId.get(flow.getBizId());
                    Long seedTypeId = invFlow == null ? gameplayCoreService.extractLongFromExtData(flow.getExtData(), "seedTypeId") : invFlow.getSeedTypeId();
                    ...
                    record.setSeedName(seedType == null ? "" : gameplayCoreService.safeString(seedType.getName()));
                    record.setItemQuantity(invFlow == null ? 0L : gameplayCoreService.defaultLong(invFlow.getChangeAmount(), 0L));
                    record.setItemType(invFlow == null ? ("BUY".equals(tradeType) ? "SEED" : "FRUIT") : gameplayCoreService.safeString(invFlow.getItemType()));
                    record.setItemOperationType(invFlow == null ? "" : gameplayCoreService.safeString(invFlow.getOperationType()));
                    record.setCoinChangeAmount(gameplayCoreService.defaultLong(flow.getChangeAmount(), 0L));
    ...
    public PageResult<SeedFruitInventoryItemVO> pageFruitInventory(SeedFruitInventoryQueryDTO query) {
        ...
        List<SeedFruitInventoryItemVO> items = userFruitDao.findByUserIdAndIsDeletedFalseOrderByIdAsc(userId).stream()
                .filter(fruit -> seedTypeMap.containsKey(fruit.getSeedTypeId()))
                .filter(fruit -> nameKeyword.isEmpty() || gameplayCoreService.safeString(seedTypeMap.get(fruit.getSeedTypeId()).getName()).toLowerCase().contains(nameKeyword))
                .map(fruit -> {
                    SeedType seedType = seedTypeMap.get(fruit.getSeedTypeId());
                    long availableQuantity = Math.max(gameplayCoreService.defaultLong(fruit.getQuantity(), 0L) - gameplayCoreService.defaultLong(fruit.getFrozenQuantity(), 0L), 0L);
                    long estimatedIncome = gameplayCoreService.safeMultiply(availableQuantity, gameplayCoreService.defaultLong(seedType.getFruitPrice(), 0L));
                    ...
                    item.setCoverImageUrl(gameplayCoreService.safeString(seedType.getCoverImageUrl()));
                    item.setFruitQuantity(gameplayCoreService.defaultLong(fruit.getQuantity(), 0L));
                    item.setFrozenQuantity(gameplayCoreService.defaultLong(fruit.getFrozenQuantity(), 0L));
                    item.setAvailableQuantity(availableQuantity);
                    item.setUnitFruitPrice(gameplayCoreService.defaultLong(seedType.getFruitPrice(), 0L));
    ...
    public SeedShopOverviewVO shopOverview(SeedShopOverviewDTO query) {
        Long userId = ServiceGuardUtils.requirePositive(query == null ? null : query.getUserId(), BizErrorCode.PARAM_INVALID, "用户ID无效");
        User user = validateUser(userId);

        Map<Long, SeedType> seedTypeMap = buildSeedTypeMap();
        List<UserFruit> fruits = userFruitDao.findByUserIdAndIsDeletedFalseOrderByIdAsc(userId);

        long sellableFruitTotalCount = fruits.stream()
                .filter(f -> seedTypeMap.containsKey(f.getSeedTypeId()))
                .mapToLong(f -> Math.max(gameplayCoreService.defaultLong(f.getQuantity(), 0L) - gameplayCoreService.defaultLong(f.getFrozenQuantity(), 0L), 0L))
                .sum();

        long sellableTotalValue = fruits.stream()
                .filter(f -> seedTypeMap.containsKey(f.getSeedTypeId()))
                .mapToLong(f -> gameplayCoreService.safeMultiply(
                        Math.max(gameplayCoreService.defaultLong(f.getQuantity(), 0L) - gameplayCoreService.defaultLong(f.getFrozenQuantity(), 0L), 0L),
                        gameplayCoreService.defaultLong(seedTypeMap.get(f.getSeedTypeId()).getFruitPrice(), 0L)
                ))
                .reduce(0L, (a, b) -> a == Long.MAX_VALUE || b == Long.MAX_VALUE ? Long.MAX_VALUE : gameplayCoreService.safeAdd(a, b));

        long currentCoin = gameplayCoreService.defaultLong(user.getCoin(), 0L);
        int purchasableSeedTypeCount = (int) seedTypeDao.findByIsDeletedFalseOrderByIdAsc().stream()
    ...
    public Long saveSeedType(SeedAddOrUpdateDTO params) {
        ...
        if (name.isEmpty()) throw new ServiceException(BizErrorCode.SEED_NAME_REQUIRED, "种子名称不能为空");

        SeedType entity;
        if (params.getId() != null && params.getId() > 0) {
            entity = ServiceGuardUtils.requirePresent(seedTypeDao.findByIdAndIsDeletedFalse(params.getId()), BizErrorCode.SEED_TYPE_NOT_FOUND, "种子类型不存在");
        } else {
            entity = new SeedType();
        ...
        ServiceGuardUtils.requirePresent(seedQualityDao.findByIdAndIsDeletedFalse(seedQualityId), BizErrorCode.SEED_QUALITY_NOT_FOUND, "种子品质不存在");

        Long enableSoilTypeBits = resolveSoilTypeBits(params);
        if (enableSoilTypeBits <= 0) throw new ServiceException(BizErrorCode.SOIL_TYPE_REQUIRED, "至少需要选择一种土壤类型");
        ...
        entity.setDescription(params.getDescription() != null ? params.getDescription() : gameplayCoreService.safeString(params.getTips()));
        entity.setMaxBugLimit(params.getMaxBugLimit() == null ? 0 : params.getMaxBugLimit());
        entity.setMaxHarvestCount(params.getMaxHarvestCount() == null ? 1 : params.getMaxHarvestCount());
        entity.setRegrowStageIndex(params.getRegrowStageIndex());
        entity.setPrice(params.getPrice() != null ? params.getPrice() : gameplayCoreService.defaultLong(params.getBuyPrice(), 0L));
        entity.setHarvestExperience(params.getHarvestExperience() != null ? params.getHarvestExperience() : gameplayCoreService.defaultLong(params.getExp(), 0L));
        entity.setHarvestFruitNumber(params.getHarvestFruitNumber() != null ? params.getHarvestFruitNumber() : (params.getHarvestCount() != null ? params.getHarvestCount() : 0));
        entity.setFruitPrice(gameplayCoreService.defaultLong(params.getFruitPrice(), 0L));
        entity.setHarvestScore(params.getHarvestScore() != null ? params.getHarvestScore() : gameplayCoreService.defaultLong(params.getScore(), 0L));
    ...
    public void removeSeedType(IdDTO params) {
        Long id = ServiceGuardUtils.requirePositive(params == null ? null : params.getId(), BizErrorCode.PARAM_INVALID, "阶段ID无效");
    ...
    public PageResult<SeedStageGridVO> pageSeedStages(SeedStageQueryDTO query) {
        Long seedTypeId = ServiceGuardUtils.requirePositive(query == null ? null : query.getSeedTypeId(), BizErrorCode.PARAM_INVALID, "种子类型ID无效");
        SeedType seedType = validateSeedType(seedTypeId);
        Map<Long, String> growthNameMap = buildGrowthStageNameMap();

        List<SeedStageGridVO> records = seedGrowthStageDao.findBySeedTypeIdAndIsDeletedFalseOrderByStageIndexAsc(seedTypeId).stream()
                .map(item -> {
                    ...
                    row.setGrowthStageName(gameplayCoreService.safeString(growthNameMap.get(item.getGrowthStageId())));
    ...
    public void saveSeedStage(SeedStageAddOrUpdateDTO params) {
        ...
        Long growthStageId = ServiceGuardUtils.requirePositive(params.getGrowthStageId(), BizErrorCode.PARAM_INVALID, "生长阶段ID无效");
        ServiceGuardUtils.requirePresent(growthStageDao.findByIdAndIsDeletedFalse(growthStageId), BizErrorCode.GROWTH_STAGE_NOT_FOUND, "生长阶段不存在");

        short stageIndex = params.getStageIndex() != null && params.getStageIndex() > 0 ? params.getStageIndex()
                : (params.getGrowthStage() == null || params.getGrowthStage() <= 0 ? 1 : params.getGrowthStage());

        SeedGrowthStage entity = (params.getId() != null && params.getId() > 0)
                ? ServiceGuardUtils.requirePresent(seedGrowthStageDao.findByIdAndIsDeletedFalse(params.getId()), BizErrorCode.SEED_STAGE_NOT_FOUND, "种子阶段配置不存在")
                : seedGrowthStageDao.findBySeedTypeIdAndStageIndexAndIsDeletedFalse(seedTypeId, stageIndex).orElseGet(() -> {
        ...
        entity.setDurationSeconds(params.getDurationSeconds() == null ? 0 : params.getDurationSeconds());
        entity.setBugProbability(params.getBugProbability() != null ? params.getBugProbability() : (params.getPestProbability() != null ? params.getPestProbability() : BigDecimal.ZERO));
    ...
    public void removeSeedStage(IdDTO params) {
        Long id = ServiceGuardUtils.requirePositive(params == null ? null : params.getId(), BizErrorCode.PARAM_INVALID, "阶段ID无效");
        SeedGrowthStage entity = ServiceGuardUtils.requirePresent(seedGrowthStageDao.findByIdAndIsDeletedFalse(id), BizErrorCode.SEED_STAGE_NOT_FOUND, "种子阶段配置不存在");
    ...
    private SeedGridVO buildSeedGridVO(SeedType seedType, Map<Long, SeedQuality> qualityMap, Map<Integer, String> soilNameByBitCode) {
        ...
        vo.setHarvestFruitNumber(seedType.getHarvestFruitNumber() == null ? 0 : seedType.getHarvestFruitNumber());
    ...
    private SeedShopItemVO buildSeedShopItemVO(SeedType seedType, Map<Long, SeedQuality> qualityMap, Map<Integer, String> soilNameByBitCode) {
        ...
        vo.setHarvestExperience(gameplayCoreService.defaultLong(seedType.getHarvestExperience(), 0L));
        vo.setHarvestScore(gameplayCoreService.defaultLong(seedType.getHarvestScore(), 0L));
        vo.setMaxHarvestCount(seedType.getMaxHarvestCount() == null ? 1 : seedType.getMaxHarvestCount());
        vo.setTotalGrowSeconds(getTotalGrowSeconds(seedType.getId()));

        long singleHarvestFruitValue = gameplayCoreService.safeMultiply(vo.getHarvestFruitNumber(), vo.getFruitPrice());
        long totalHarvestFruitValue = gameplayCoreService.safeMultiply(singleHarvestFruitValue, vo.getMaxHarvestCount());
    ...
    private Map<Integer, String> buildSoilNameByBitCode() {
        return soilTypeDao.findByIsDeletedFalseOrderByIdAsc().stream()
                .collect(Collectors.toMap(SoilType::getBitCode, s -> gameplayCoreService.safeString(s.getName()), (v1, v2) -> v1, LinkedHashMap::new));
    }

    private Map<Long, String> buildGrowthStageNameMap() {
        return growthStageDao.findByIsDeletedFalseOrderByIdAsc().stream()
                .collect(Collectors.toMap(GrowthStage::getId, s -> gameplayCoreService.safeString(s.getName())));
    }

    private String resolveSeedQualityName(Map<Long, SeedQuality> qualityMap, Long seedQualityId) {
        return qualityMap.containsKey(seedQualityId) ? gameplayCoreService.safeString(qualityMap.get(seedQualityId).getName()) : "";
    }
    ...
    private int getTotalGrowSeconds(Long seedTypeId) {
        if (seedTypeId == null || seedTypeId <= 0) return 0;
        return seedGrowthStageDao.findBySeedTypeIdAndIsDeletedFalseOrderByStageIndexAsc(seedTypeId).stream()
    ...
    private Comparator<SeedShopItemVO> resolveShopComparator(String sort, String order) {
        Comparator<SeedShopItemVO> comparator = switch (gameplayCoreService.safeString(sort).trim().toLowerCase()) {
            case "price"
                    -> Comparator.comparing(item -> gameplayCoreService.defaultLong(item.getPrice(), 0L));
            case "fruitprice"
                    -> Comparator.comparing(item -> gameplayCoreService.defaultLong(item.getFruitPrice(), 0L));
            case "level"
                    -> Comparator.comparing(item -> item.getLevel() == null ? 0 : item.getLevel().intValue());
            case "totalharvestfruitvalue"
                    -> Comparator.comparing(item -> gameplayCoreService.defaultLong(item.getTotalHarvestFruitValue(), 0L));
            case "estimatednetvalue"
                    -> Comparator.comparing(item -> gameplayCoreService.defaultLong(item.getEstimatedNetValue(), 0L));
            default
    ...
    private Long resolveSoilTypeBits(SeedAddOrUpdateDTO params) {
        if (params.getEnableSoilTypeBits() != null && params.getEnableSoilTypeBits() > 0) return params.getEnableSoilTypeBits();

        String idStr = gameplayCoreService.safeString(params.getSoilTypeIds()).trim();
        if (idStr.isEmpty()) idStr = gameplayCoreService.safeString(params.getSoilTypeId()).trim();
    ...
    private Short resolveSeedLevel(SeedAddOrUpdateDTO params) {
        if (params.getLevel() != null && params.getLevel() > 0) return params.getLevel();
    ...
    private SeedType validateSeedType(Long seedTypeId) {
        return ServiceGuardUtils.requirePresent(seedTypeDao.findByIdAndIsDeletedFalse(seedTypeId), BizErrorCode.SEED_TYPE_NOT_FOUND, "种子类型不存在");
    }

    private UserSeed validateUserSeed(Long userId, Long seedTypeId) {
        return ServiceGuardUtils.requirePresent(userSeedDao.findByUserIdAndSeedTypeIdAndIsDeletedFalse(userId, seedTypeId), BizErrorCode.SEED_INVENTORY_NOT_FOUND, "种子库存不存在");
    }

    private UserFruit validateUserFruit(Long userId, Long seedTypeId) {
        return ServiceGuardUtils.requirePresent(userFruitDao.findByUserIdAndSeedTypeIdAndIsDeletedFalse(userId, seedTypeId), BizErrorCode.FRUIT_INVENTORY_NOT_FOUND, "果实库存不存在");
    }
}

...
public class GameplayLogQueryServiceImp implements GameplayLogQueryService {
    ...
    public PageResult<CropActionLogRecordVO> pageCropActionLogs(CropActionLogQueryDTO params) {
        ...
        if (request.getActionType() != null && !request.getActionType().isBlank() && actionTypeFilter.isBlank()) {
            throw new ServiceException(BizErrorCode.CROP_ACTION_TYPE_UNSUPPORTED, "actionType 仅支持 CARE / CLEAR / HARVEST / PLANT / BUG_SPAWN");
        }
        ...
        List<UserCropActionLog> actionLogs = userCropActionLogDao.findByUserIdAndIsDeletedFalseOrderByActionAtDesc(request.getUserId());
        List<CropActionLogRecordVO> records = new ArrayList<>();
        for (UserCropActionLog actionLog : actionLogs) {
            String actionType = gameplayCoreService.safeString(actionLog.getActionType()).trim().toUpperCase();

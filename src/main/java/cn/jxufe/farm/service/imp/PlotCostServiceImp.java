...
public class PlotCostServiceImp implements PlotCostService {
  ...
  public PlotCostServiceImp(GameplayPolicyProperties gameplayPolicyProperties) {
  ...
  public long calculateUnlockCostCoin(Short plotIndex) {
    ...
    return gameplayPolicyProperties.getPlot().getUnlock().getBaseCostCoin()
        + (safePlotIndex - freeLimit - 1L)
            * gameplayPolicyProperties.getPlot().getUnlock().getCostStepCoin();
  }
}

...
public class UserServiceImp implements UserService {
  ...
  private static final String DEFAULT_PREFERENCES_JSON =
      "{\"audio\":{\"effectEnabled\":true,\"effectVolume\":0.8,\"bgmEnabled\":true,\"bgmVolume\":0.6}}";
  ...
  public PageResult<UserInfoVO> list() {
    List<UserInfoVO> rows =
        userDao.findByIsDeletedFalseOrderByIdAsc().stream()
  ...
  public PageResult<UserInfoVO> gridDataFilterSortPage(String name, PageQueryDTO pageRequest) {
    ...
    Pageable pageable =
        PageRequest.of(pageNumber, pageSize, Sort.by(direction, safeSortField(request.getSort())));

    Page<User> userPage =
        userDao.findByIsDeletedFalseAndUsernameContainingIgnoreCase(keyword, pageable);

    List<UserInfoVO> rows =
        userPage.getContent().stream().map(this::buildUserInfoVO).collect(Collectors.toList());

    return new PageResult<>(
        request.getPage().longValue(), pageSize, userPage.getTotalElements(), rows);
  }
  ...
  public UserInfoVO addOrUpdate(UserAddOrUpdateDTO params) {
    ...
    entity.setExperience(
        Math.max(0L, parseLong(params.getExperience(), parseLong(params.getExp(), 0L))));
  ...
  public void delete(IdDTO params) {
    Long id =
        ServiceGuardUtils.requirePositive(
            params == null ? null : params.getId(), BizErrorCode.PARAM_INVALID, "用户ID无效");
    User user =
        ServiceGuardUtils.requirePresent(
            userDao.findByIdAndIsDeletedFalse(id), BizErrorCode.USER_NOT_FOUND, "用户不存在");
  ...
  public CurUserVO setCurUser(HttpSession session, SetCurUserDTO user) {
    Long userId =
        ServiceGuardUtils.requirePositive(
            user == null ? null : user.getId(), BizErrorCode.PARAM_INVALID, "用户ID无效");
    User curUser =
        ServiceGuardUtils.requirePresent(
  ...
  public UserSettingsVO saveCurUserSettings(HttpSession session, UserSettingsUpdateDTO params) {
    ...
    ObjectNode audio = objectNode(root, "audio");
  ...
  private User requireSessionUser(HttpSession session) {
    ...
    return ServiceGuardUtils.requirePresent(
        userDao.findByIdAndIsDeletedFalse(user.getId()), BizErrorCode.USER_NOT_FOUND, "当前用户不存在");
  }
  ...
  private String normalizeAvatarPath(String avatarPath) {
    ...
    return input.isBlank()
        ...
        : FileAccessPathUtils.normalizeIncomingRelativePath(
            input, fileStorageProperties.getPublicPrefix());
  }
  ...
  private void initDefaultPlotsForNewUser(Long userId) {
    ...
    if (!userPlotDao.findByUserIdAndIsDeletedFalseOrderByPlotIndexAsc(userId).isEmpty()) {
      return;
    }
    ...
    PlotPolicy activePolicy =
        plotPolicyDao.findFirstByActiveTrueAndIsDeletedFalseOrderByIdAsc().orElse(null);
    short totalPlotCount =
        (short)
            Math.max(
                1,
                activePolicy != null
                    ? activePolicy.getDefaultTotalPlotCount()
                    : gameplayPolicyProperties.getPlot().getDefaults().getTotalPlotCount());
    short unlockedPlotCount =
        (short)
            clamp(
                activePolicy != null
                    ? activePolicy.getDefaultUnlockedPlotCount()
                    : gameplayPolicyProperties.getPlot().getDefaults().getUnlockedPlotCount(),
                1,
                totalPlotCount);

    List<UserPlot> initPlots =
        IntStream.rangeClosed(1, totalPlotCount)
            .mapToObj(
                index -> {
                  ...
                  plot.setUnlockExperienceRequired(
                      calculatePlotUnlockRequiredExperience((short) index));

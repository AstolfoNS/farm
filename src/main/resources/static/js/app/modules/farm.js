...
function renderDialogTemplate(options) {     var opts = $.extend(       {         dialogSelector: "",         title: "",         infoSelector: "",         rows: [],         actionSelector: "",         buttons: [],       },       options || {},     );      if (ActionKit && $.isFunction(ActionKit.renderDialogTemplate)) {       ActionKit.renderDialogTemplate(opts);       return;     }      if (opts.dialogSelector && opts.title) {       $(opts.dialogSelector).dialog("setTitle", opts.title);     }     var html = [];     $.each(opts.rows || [], function (_, row) {       if (!row) {         return true;       }       var content =         row.html != null           ? String(row.html)           : row.label             ? escapeHtml(row.label) + ": " + escapeHtml(row.value)             : escapeHtml(row.value);       if (row.strong === true) {         content = "<strong>" + content + "</strong>";       }       var rowClass =         "farm-action-row" + (row.className ? " " + row.className : "");       html.push("<div class='" + rowClass + "'>" + content + "</div>");       return true;     });     $(opts.infoSelector).html(html.join(""));      var buttonHtml = [];     $.each(opts.buttons || [], function (_, btn) {       if (!btn || btn.visible === false || !btn.action || !btn.text) {         return true;       }       var className = "easyui-linkbutton" + (btn.skin ? " " + btn.skin : "");       var attrs = btn.id ? " id='" + btn.id + "'" : "";       buttonHtml.push(         "<a href='javascript:void(0)' class='" +           className +           "' data-action='" +           btn.action +           "'" +           attrs +           ">" +           escapeHtml(btn.text) +           "</a>",       );       return true;     });     $(opts.actionSelector).html(buttonHtml.join(""));     $(opts.actionSelector + " .easyui-linkbutton").linkbutton();   }    function findPlot(plotId) {     var target = String(plotId || "");     var result = null;     $.each(asArray(state.overview && state.overview.plots), function (_, plot) {       if (String(plot.plotId || "") === target) {         result = plot;         return false;       }       return true;     });     return result;   }    function animatePlotAction(plotId, actionType, tipText) {
...
function loadSoilOptions(onDone) {     if ($.isArray(state.soilOptions) && state.soilOptions.length > 0) {       onDone(state.soilOptions);       return;     }     FarmApi.listSoilOptions(       function (res) {         if (!(FarmApi.isOk(res) && $.isArray(res.data))) {           onDone([]);           return;         }         state.soilOptions = res.data;         onDone(state.soilOptions);       },       function () {         onDone([]);       },     );   }    function executeExpand() {
      ...
      confirmAction({
        ...
        onConfirm: function () {
          runFarmAction({
            ...
            failMessageBuilder: function (res) {
              return buildFarmActionFailMessage("扩地", {
                plotIndex: 0,
                crop: {seedTypeName: soils[0] && soils[0].name ? soils[0].name : "默认土壤"}
              }, res && res.msg);
            },
...
function applyToolOnPlot(plotId) {     var plot = findPlot(plotId);     if (!plot) {       return;     }     var mode = state.selectedTool || "inspect";     if (mode === "inspect") {       openPlotActionDialog(plotId);       return;     }     if (mode === "plant") {       if (plot.locked) {         showActionError("该地块尚未解锁，请先在查看模式中完成解锁。");         return;       }       if (plot.hasCrop) {         showActionError("该地块已有作物，当前不能继续播种。");         return;       }       openSeedDialog(plot);       return;     }     if (mode === "care") {       if (plot.locked || !plot.hasCrop) {         showActionError("该地块当前没有可养护的作物。");         return;       }       if (!(plot.crop && plot.crop.canCare)) {         showActionError("当前作物没有虫害，不需要执行杀虫。");         return;       }       executeCare(plot);       return;     }     if (mode === "harvest") {       if (plot.locked || !plot.hasCrop) {         showActionError("该地块当前没有可收获的作物。");         return;       }       if (!(plot.crop && plot.crop.harvestable)) {         showActionError("当前作物尚未成熟，暂时不能收获。");         return;       }       executeHarvest(plot);       return;     }     if (mode === "clean") {       if (plot.locked || !plot.hasCrop) {         showActionError("该地块当前没有可铲除的作物。");         return;       }       confirmAction({         title: "确认铲除",         message:           plot.crop && plot.crop.harvestable             ? "该作物已经成熟，确认直接铲除吗？"             : "该作物尚未成熟，确认直接铲除吗？",         detail:           "铲除后不会获得果实、经验与积分。" +           (plot.crop && plot.crop.harvestable             ? "建议先使用“收获”工具完成收获。"             : ""),         onConfirm: function () {           executeClear(plot);         },       });       return;     }     openPlotActionDialog(plotId);   }    function setActive(flag) {     state.active = !!flag;     if (state.active) {       $("#farmPanel").addClass("is-active is-module-enter");       window.setTimeout(function () {         $("#farmPanel").removeClass("is-module-enter");       }, motion().moduleEnterMs);       ensureSeedVisuals();       switchTool(state.selectedTool || "inspect");       loadOverviewByUser(currentUserId(), false);       if (window.FarmWsBridge && window.FarmWsBridge.isConnected()) {         onRealtimeStatus("connected");       } else {         onRealtimeStatus(state.wsStatus || "idle");       }       return;     }     $("#farmPanel").removeClass("is-active");     stopPolling();   }    function bindRealtime() {     $(document)       .off("farm:realtime:overview.farm")       .on("farm:realtime:overview.farm", function (_, payload) {         if (!(payload && payload.overview)) {           return;         }         var loginUid = currentUserId();         var pushUid = asNumber(payload.userId, 0);         if (loginUid <= 0 || (pushUid > 0 && pushUid !== loginUid)) {           return;         }         updateOverview(payload.overview, false);       });      $(document)       .off("farm:realtime:status.farm")       .on("farm:realtime:status.farm", function (_, text) {         onRealtimeStatus(text);       });   }    function bindEvents() {     $("#farmFallbackSwitch")       .off("change.farm")       .on("change.farm", function () {         fallbackEnabled();         if ($(this).prop("checked")) {           startPolling();         } else {           stopPolling();         }       });      $("#farmToolBar")       .off("click.farm", ".farm-tool-btn")       .on("click.farm", ".farm-tool-btn", function () {         switchTool($(this).attr("data-tool"));         playSound("click");       });      $("#farmIsoContainer")       .off("click.farm", ".farm-plot")       .on("click.farm", ".farm-plot", function () {         var plotId = $(this).attr("data-plot-id");         applyToolOnPlot(plotId);       });   }    FarmModule.setActive = setActive;   FarmModule.loadOverviewByUser = loadOverviewByUser;   FarmModule.updateOverview = updateOverview;    window.FarmModule = FarmModule;   if (     window.FarmCore &&     $.isFunction(window.FarmCore.registerSetActiveModule)   ) {     window.FarmCore.registerSetActiveModule("farm", FarmModule, {       refresh: function () {         loadOverviewByUser(currentUserId(), false);       },     });   }    $(function () {     bindRealtime();     bindEvents();     renderAll({ plots: [] });     onRealtimeStatus("idle");   }); })(window, window.jQuery); 